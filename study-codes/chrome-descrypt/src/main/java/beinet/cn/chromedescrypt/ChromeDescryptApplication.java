...
import beinet.cn.chromedescrypt.chromeTest.ChromeCookieReader;
...
public class ChromeDescryptApplication {
    ...
    /**
     * 读取并解密Chrome存储的账号密码。
     * 注：所有基于Chromium内核的账号密码，都可以使用这段代码读取。
     */
    ...
    /**
     * 读取并解密Chrome存储的Cookie。
     * 注：所有基于Chromium内核的Cookie，都可以使用这段代码读取。
     */
    private static void readAndShowChromeCookie() {
        /*
         * 账号密码存储的文件路径，在
         ...
         * 注：建议关闭Chrome后再读取，也可以把它拷贝出来进行读取测试
         */
        String localCookieDB = "D:/Cookies";
        /*
         * 上面的Cookie的值是加密了的，解密的密码放在另一个文件里，文件路径在
         * %LocalAppData%\Google\Chrome\User Data\Local State"
         * 这是一个json格式的文件，密码在 os_crypt.encrypted_key 属性里.
         *
         * 注意：只能在本机运行该程序，解密其它机器上的文件会抛异常：
         ...
         */
        String localStateFile = "D:/Local State";
        var result = new ChromeCookieReader(localCookieDB, localStateFile).queryData();
        for (var item : result) {
            // 输出解密后的账号密码
            System.out.println(item);
        }
